...
def _get_company():
    ...
    co = frappe.form_dict.get("company", "All") or "All"
    keys = [k for k in frappe.form_dict.keys() if k not in ("cmd", "user", "sid", "_")]
    frappe.logger().info(f"OIL CMD: company={co!r} extra_keys={keys}")
    return co
...
def get_kpis():
    ...
    sales = frappe.db.sql(
        ...
        WHERE docstatus = 1 AND posting_date >= %s """ + co_filter,
        (get_first_day(today()),) + tuple(co_args), as_dict=True,
    )
    ...
    if company and company != "All":
        ict = frappe.db.sql(
            ...
            (get_first_day(today()), company, company), as_dict=True,
        )
    else:
        ict = frappe.db.sql(
            """SELECT COALESCE(SUM(total_qty), 0) as qty
            FROM `tabInter Company Transfer`
            WHERE docstatus = 1 AND posting_date >= %s""",
            (get_first_day(today()),), as_dict=True,
        )
...
def get_sales_procurement_trend():
    ...
    for i in range(5, -1, -1):
        ...
        sales = frappe.db.sql(
            ...
            + co_filter,
            (month_start_dt, month_end_dt) + tuple(co_args), as_dict=True,
        )
...
def get_company_stock_distribution():
    ...
    if company and company != "All":
        data = frappe.db.sql(
            ...
            FROM `tabBin` b JOIN `tabWarehouse` w ON w.name = b.warehouse
            WHERE w.name LIKE 'Available WH - %%' AND b.actual_qty > 0 AND w.company = %s
            GROUP BY w.company ORDER BY qty DESC""",
            (company,), as_dict=True,
        )
    else:
        data = frappe.db.sql(
...
def get_negative_stock():
    ...
    if company and company != "All":
        ...
        return frappe.db.sql(
            """SELECT w.company, w.name as warehouse, b.item_code, b.actual_qty, b.stock_value
            FROM `tabBin` b JOIN `tabWarehouse` w ON w.name = b.warehouse
            WHERE b.actual_qty < 0 ORDER BY b.actual_qty ASC LIMIT 20""",
            as_dict=True,
        )
...
def get_recent_reservations():
    ...
    if company and company != "All":
        return frappe.db.sql(
            ...
            WHERE docstatus = 1 AND status = 'Reserved' AND company = %s
...
def get_recent_icts():
    ...
    if company and company != "All":
        return frappe.db.sql(
            ...
            FROM `tabInter Company Transfer`
            WHERE docstatus = 1 AND (company = %s OR to_company = %s)
