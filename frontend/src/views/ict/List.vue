  ...
  <ion-page>
    <ion-header class="ion-no-border">
      <ion-toolbar>
        ...
        <ion-title>All Transfers</ion-title>
      </ion-toolbar>
      <ion-toolbar>
        <ion-searchbar :value="search" @ionInput="search = $event.detail.value" placeholder="Search transfers..." animated></ion-searchbar>
      </ion-toolbar>
    </ion-header>
    <ion-content>
      ...
      <ion-list class="inset-list" v-if="filtered.length">
        <ion-item v-for="t in filtered" :key="t.name" button @click="openDoc(t.name)">
          <ion-label>
            <div class="item-title-row"><span class="item-title">{{ t.name }}</span><ion-badge :color="badgeColor(t.status)" class="item-badge">{{ t.status }}</ion-badge></div>
            <ion-note class="item-sub">{{ abbr(t.company) }} → {{ abbr(t.to_company) }} &middot; {{ formatDate(t.posting_date) }}</ion-note>
          </ion-label>
...
import { formatCurrency, formatDate, abbr } from "@/utils/formatters"
...
const filtered = computed(() => {
  ...
  return transfers.value.filter(t => t.name.toLowerCase().includes(q) || t.company.toLowerCase().includes(q) || t.to_company.toLowerCase().includes(q))
})
...
async function loadData() {
  loading.value = true
  try { transfers.value = (await frappe.call({ method: "oil_distribution.api.oil_ops.get_ict_list", args: { company: "All", limit: 100 } })).message || [] } catch (e) { console.error(e) } finally { loading.value = false }
}
